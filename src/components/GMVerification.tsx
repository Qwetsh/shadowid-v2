...
export const GMVerification: React.FC<GMVerificationProps> = ({ onClose, isMobile = false }) => {
  ...
  return (
    <div className={isMobile ? "fixed inset-0 bg-cyber-darker flex flex-col z-50 safe-area-inset" : "fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4"}>
      <div className={isMobile ? "w-full h-full flex flex-col" : "bg-cyber-darker border-2 border-cyber-neon-cyan rounded-lg max-w-md w-full p-6 max-h-[90vh] overflow-y-auto font-cyber"}>
        {/* Header */}
        <div className={`flex items-center justify-between ${isMobile ? "p-4 pb-2 border-b border-cyber-border" : "mb-6"}`}>
          <h2 className={`${isMobile ? "text-2xl" : "text-xl"} font-bold text-cyber-neon-cyan uppercase`}>
            GM ID Verification
          </h2>
          {isMobile ? (
            <div className="text-xs text-cyber-neon-yellow">🛡️ VERIFICATION MODE</div>
          ) : (
            <button
              onClick={onClose}
        ...
        <div className={isMobile ? "flex-1 overflow-y-auto" : ""}>
          {/* Instructions */}
          {!qrData && !result && (
            <div className={`space-y-4 ${isMobile ? "p-4" : "mb-6"} text-sm text-cyber-neon-cyan`}>
              <p className="font-bold text-cyber-neon-pink text-lg">📱 How to Use:</p>
              ...
              <p className="text-sm text-cyber-neon-yellow font-bold">
                💡 Lower SIN ratings = higher detection chances!
              </p>
            </div>
          )}
          ...
          {/* QR Data Input */}
          {!result && (
            <div className={`space-y-3 ${isMobile ? "p-4" : "mb-6"}`}>
              <div className="flex gap-2 flex-col">
                <button
                  onClick={() => setShowScanner(true)}
                  className={`px-4 ${isMobile ? "py-4 text-lg" : "py-2 text-sm"} bg-cyber-neon-green text-cyber-darker font-bold rounded hover:opacity-80 transition-opacity uppercase flex items-center justify-center gap-2 w-full`}
                >
                  <Camera size={isMobile ? 24 : 16} />
                  Scan Camera
                </button>
              </div>
          ...
          {/* Error Message */}
          {error && (
            <div className={`${isMobile ? "m-4" : "mb-4"} p-3 bg-cyber-neon-pink bg-opacity-20 border border-cyber-neon-pink rounded text-cyber-neon-pink text-xs`}>
              {error}
            </div>
          )}
          ...
          {verificationStatus && (
            <div
              className={`${isMobile ? "m-4" : "mb-4"} p-3 rounded text-xs font-bold`}
              style={{
          ...
          {/* QR Data Display */}
          {qrData && (
            <div className={`space-y-3 ${isMobile ? "m-4 p-4 text-base" : "mb-6 p-4 text-xs"} bg-cyber-darker border border-cyber-border rounded`}>
              <div>
              ...
              <div>
                ...
                <span className="ml-2 font-bold text-lg">{qrData.sinRating}/6</span>
              </div>
              <div>
          ...
          {result && (
            <div className={`space-y-4 ${isMobile ? "p-4" : ""}`}>
              ...
              >
                <div
                  className={`${isMobile ? "text-5xl mb-4" : "text-3xl mb-3"} font-bold uppercase`}
                  style={{
                ...
                <div className={`${isMobile ? "text-base" : "text-xs"} text-cyber-neon-cyan font-bold`}>
                  Detection Probability: {result.probability.toFixed(0)}%
                </div>
              </div>

              {/* Additional Info */}
              <div className={`p-3 bg-cyber-border bg-opacity-30 rounded ${isMobile ? "text-base" : "text-xs"} text-cyber-neon-yellow`}>
              ...
              <div className={`flex gap-2 ${isMobile ? "flex-col" : ""}`}>
                <button
                  ...
                  className={`${isMobile ? "py-3 text-base" : "py-2 text-xs"} px-4 bg-cyber-neon-cyan text-cyber-darker font-bold rounded hover:opacity-80 transition-opacity uppercase flex-1`}
                >
                  Verify Another
                </button>
                {!isMobile && (
        ...
        {isMobile && result && (
          <div className="border-t border-cyber-border p-4 mt-4">
            <button
              onClick={onClose}
              className="w-full py-3 px-4 bg-cyber-border text-cyber-neon-cyan font-bold rounded hover:opacity-80 transition-opacity uppercase text-base"
            >
              Close Verification
            </button>
          </div>
